...
/// Visualizes the real-time agentic reasoning pipeline for pulseAI.
...
class _LiveTraceScreenState extends State<LiveTraceScreen>
  ...
  void _initConnection() {
    // Get runId from arguments — accept either String or {'run_id': ...}
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (rawArgs is String) {
        ...
        runId = (rawArgs['run_id'] as String?) ?? 'DEMO_RUN';
      }
  ...
  void _updateAgentStep(String? agent, String? kind) {
    ...
    const agentMap = {
      ...
      'contradictions': 'ConflictResolver',
    ...
    final targetAgent = agentMap[agent.toLowerCase()] ?? agent;

    if (!_agentStates.containsKey(targetAgent)) return;

    final kindLower = kind?.toLowerCase() ?? '';
    final isComplete = kindLower == 'completed' ||
        kindLower == 'resource_ready' ||
        kindLower == 'phase_complete' ||
        kindLower == 'resource_ready_replay';
    final isStarted = kindLower == 'started';
      ...
      child: InkWell(
        onTap: () {
          ...
          if (rawArgs is String) {
            ...
            runId = rawArgs['run_id'] as String?;
          }
